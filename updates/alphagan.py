...
class AlphaGAN(nn.Module):
	...
	def autoencoder_loss(self, x, y):
		...
		ret = {}
		if self.use_C():
			# ret['code_adversarial_loss'] = - code_adv_beta_vae * (self.C(z) + _eps).log().mean()
			cel = nn.CrossEntropyLoss()
			ret['code_adversarial_loss'] = code_adv_beta_vae * cel(self.C(z), y)
		if self.adversarial_weight != 0:
			cel = nn.CrossEntropyLoss()
			ret['adversarial_loss'] = cel(self.D(x_fake), torch.Tensor(x_fake.size(0)).fill_(10).type(torch.LongTensor).to('cuda:0'))
	...
	def discriminator_loss(self, x, y):
		...
		return {
			'discriminator_loss':
				cel(self.D(x), y) + cel(self.D(x_fake), torch.Tensor(x_fake.size(0)).fill_(10).type(torch.LongTensor).to('cuda:0'))
				# - (self.D(x) + _eps).log().mean()
				# - (1 - self.D(x_fake) + _eps).log().mean()

		}

	def code_discriminator_loss(self, x, y):
		...
		return {
			'code_discriminator_loss':
				# code_discrim_beta_vae * (- (self.C(z_prior) + _eps).log().mean()
				# - (1 - self.C(z) + _eps).log().mean())
				code_discrim_beta_vae * (cel(self.C(z), y) + cel(self.C(z_prior), torch.Tensor(z_prior.size(0)).fill_(10).type(torch.LongTensor).to('cuda:0')))
	...
	def _epoch(self, X, loss_fns,
		...
		for x_sample in pbar(it, desc=desc, leave=False):
			x = self._wrap(x_sample[0])
			y = self._wrap(x_sample[1])
			# print(y)
			for opt, iters, loss_fn in zip(optimizers, n_iter, loss_fns):
				for _ in range(iters):
					loss_components = loss_fn(x, y)
					if opt:
